...
def check(url):
    ...
    try:
        ...
        if 'Dashboard' in resp:
            ...
            print(' -| {:<50} --> {}[Login Successfully | Add Plugins: {} | Theme Editor: {}]'.format(url, fg, 'Yes' if has_add_plugins else 'No', 'Yes' if has_theme_editor else 'No'))
            open("result.txt", "a").write(f"{site}:{user}:{passwd} | Add Plugins: {'Yes' if has_add_plugins else 'No'} | Theme Editor: {'Yes' if has_theme_editor else 'No'}\n")            
        else:
            print(' -| {:<50} --> {}[Login Failed]'.format(url, fr))
